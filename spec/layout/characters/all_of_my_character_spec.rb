...
RSpec.feature 'Turn up with user and all fo the character details history' do
  ...
  scenario 'User has character alive' do
    ...
    expect(page).to have_link('Manage Character')
    expect(page).to have_content('Status:alive', count: 1)
    expect(page).to have_content('Name of Character:', count: all_of_character_of_user_all_of_my_c)
    expect(page).to have_content('Was living:', count: all_of_character_of_user_all_of_my_c - 1)
    expect(page).to have_content('Status:dead', count: all_of_character_of_user_all_of_my_c - 1)
    expect(page).to have_content('Your characters history')
