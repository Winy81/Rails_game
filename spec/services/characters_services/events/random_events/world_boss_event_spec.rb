...
describe CharactersServices::Events::RandomEvents::WorldBossEvent do

  let(:id) { CharactersServices::Events::RandomEvents::WorldBossEvent::ID }
  let(:gold_for_reach) { CharactersServices::Events::RandomEvents::WorldBossEvent::GOLD_FOR_REACH }
  let(:description) { CharactersServices::Events::RandomEvents::WorldBossEvent::DESCRIPTION }
  let(:activity_for_lost) { CharactersServices::Events::RandomEvents::WorldBossEvent::REDUCED_ACTIVITY_LEVEL_WITH }
  ...
  context 'When the event has been called' do
    ...
    it 'should update the characters owner budget with GOLD_FOR_REACH and the characters activity with REDUCED_ACTIVITY_LEVEL_WITH' do
      ...
      CharactersServices::Events::RandomEvents::WorldBossEvent.new().process
